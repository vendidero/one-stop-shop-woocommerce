...
class Tax {
	...
	public static function vat_exempt_taxable_address( $location ) {
		if ( \Vendidero\TaxHelper\Tax::is_vat_exempt() ) {
			$location = array(
	...
	public static function disable_location_price() {
		...
		if ( $fixed_gross_prices ) {
			$tax_location = \Vendidero\TaxHelper\Tax::get_taxable_location();
			...
			if ( ! empty( $tax_location[0] ) ) {
				 ...
				 */
				if ( ! \Vendidero\TaxHelper\Tax::country_supports_eu_vat( $country, $postcode ) && apply_filters( 'oss_disable_static_gross_prices_third_countries', ( 'yes' !== get_option( 'oss_fixed_gross_prices_for_third_countries' ) ), $tax_location ) ) {
	...
	public static function filter_tax_class( $tax_class, $product ) {
		$taxable_address = \Vendidero\TaxHelper\Tax::get_taxable_location();

		if ( isset( $taxable_address[0] ) && ! empty( $taxable_address[0] ) && \Vendidero\TaxHelper\Tax::get_base_country() !== $taxable_address[0] ) {
	...
	protected static function get_selectable_countries() {
		$countries = \Vendidero\TaxHelper\Tax::get_non_base_eu_countries( true );
	...
	public static function get_product_tax_class_by_country( $product, $address = array(), $default = false ) {
		...
		if ( apply_filters( 'oss_woocommerce_switch_product_tax_class', $filter_tax_class, $product, $address['country'], $postcode, $default ) ) {
			...
			if ( false === $matched_tax_class ) {
				$tax_classes     = self::get_product_tax_classes( $product );
				$tax_class_slugs = \Vendidero\TaxHelper\Tax::get_tax_class_slugs();
